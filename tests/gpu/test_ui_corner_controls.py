...
from mojive.tools.ui_feasibility import workspace as probe_workspace
...
def test_corner_sliders_update_independently_and_keep_fractional_values(monkeypatch):
    ...
    try:
        probe_fixtures._apply_concept_theme(window.style_scale)
        state = probe_state.ProbeState(page="Geometry", geometry_tab="Corners")
        ...
        def frame():
            window.begin_frame()
            probe_workspace._draw_workspace(window, state)
            window.end_frame()

        for _ in range(4):
            frame()
        assert set(rectangles) == {
            f"##corner-{name}" for _, name in probe_fixtures.CORNER_CONTROLS
        } | {"##imgui-corner-radius"}
        for index, (_, name) in enumerate(probe_fixtures.CORNER_CONTROLS):
            before = {key: getattr(state, key) for _, key in probe_fixtures.CORNER_CONTROLS}
        ...
        assert before == {key: getattr(state, key) for _, key in probe_fixtures.CORNER_CONTROLS}
        for name in theme_mod.CONTROL_ROUNDING:
        ...
        exported = probe_tuning._geometry_values_text(state)
        assert f"imgui_rounding={state.imgui_rounding}," in exported
        assert all(
            f"{name}={getattr(state, name)}," in exported
            for _, name in probe_fixtures.CORNER_CONTROLS
        )
    finally:
        window.close()
