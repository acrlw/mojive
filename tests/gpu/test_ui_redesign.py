...
class Rig:
    def __init__(self, monkeypatch, width=1400, height=820, scale=1.0, language="en"):
        ...
        probe_fixtures._apply_concept_theme(self.window.style_scale)
        self.probe = probe_state.ProbeState(page="Redesign")
    ...
    def frame(self):
        ...
        probe_workspace._draw_workspace(self.window, self.probe)
...
def test_keyframe_follow_icons_keep_native_selection_in_production_and_preview(monkeypatch, scale):
    ...
    def record_tooltip(label):
        if label in keyframes_panel_module.FOLLOW_MODE_TOOLTIPS and imgui.is_item_hovered():
...
def test_existing_capsule_ink_matches_geometry_and_reset_stays_in_bound(rig, monkeypatch):
    ...
    def draw(_window, state):
        ...
        if redesign:
            _capsule(
                state.redesign, (100, 100), 2, state, probe_widgets._circular_icon_button, vertical
            )
        elif vertical:
            probe_widgets._draw_tool_column(
                ImguiDraw2D(imgui.get_window_draw_list()), (100, 100), 2, state
            )
        else:
            probe_widgets._draw_playback(
                ImguiDraw2D(imgui.get_window_draw_list()), (100, 100), 2, state
            )
        imgui.end()

    monkeypatch.setattr(probe_workspace, "_draw_workspace", draw)
    ...
    for stroke in (1.0, OVERLAY_GEOMETRY.tool_stroke, 2.2):
...
def test_capsules_and_narrow_fields_fit_at_fractional_scales(monkeypatch, scale, language):
    ...
    try:
        for radial_step in (10, OVERLAY_GEOMETRY.radial_step):
