...
from mojive.geometry2d.curves import capped_polyline_points
...
from mojive.ui.imgui_draw import ImguiDraw2D, _anti_alias_fringe_outer
...
from mojive.geometry2d.curves import arrow_points
...
@pytest.mark.parametrize("width", [0, 0.25, 0.5, 1.0])
@pytest.mark.parametrize("shape", ["open", "closed", "circle", "rect"])
def test_subpixel_native_strokes_preserve_coverage_instead_of_a_bright_pixel_floor(
    native_draw, width, shape
):
    color = (1, 1, 1, 0.8)
    if shape == "circle":
        native_draw.circle((20, 20), 10, color, width)
    elif shape == "rect":
        native_draw.rect((10, 10), (30, 30), color, width)
    else:
        native_draw.polyline(((10, 10), (30, 10), (30, 30)), color, width, closed=shape == "closed")
    peak = max((vertex.col >> 24 for vertex in native_draw._dl.vtx_buffer), default=0)
    assert peak == pytest.approx(255 * 0.8 * width, abs=1)
...
def test_arrow_motion_reuses_local_mesh_and_antialias_preparation(native_draw):
    from mojive.geometry2d.curves import arrow_mesh
    from mojive.ui.imgui_draw import _cached_fringe_points
