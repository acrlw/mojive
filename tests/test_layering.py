...
@pytest.mark.parametrize("package", ("geometry2d", "text"))
def test_geometry_and_text_are_independent_of_draw_adapters_and_interaction(package):
    forbidden = (
        "mojive.draw2d",
        "mojive.ui",
        "mojive.render",
...
def test_session_capture_and_rpc_do_not_import_physics_or_compatibility_renderer():
    ...
    for name in (
        ...
        *(str(path.relative_to(SRC)) for path in _files("control/rpc")),
        "session/capture.py",
        "render/offscreen.py",
    ):
