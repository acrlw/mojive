...
class TimelineEditor(TimelineEditorState):
    ...
    def begin_timeline_edit(
        ...
        *,
        ...
        if snapshot_hit >= 0:
            self.select_snapshot(snapshot_hit)
            if (
                load
            ...
            if load and self.can_load_keyframes(ctx):
            ...
            if not ctx.session.state_take_recording:
                ...
                self.selection_generation = -1
                if self.pointer_mode == "select" and lane == "take":
    ...
    def update_timeline_drag(
        ...
        *,
        ...
        if self.pointer_mode == "select" and pointer.held(self.pointer_chord):
            ...
            self.select_to(time, self.marker_index, take_times)
        ...
        if self.drag_id >= 0 and pointer.held(self.pointer_chord):
            ...
            preview = self.preview_key(
                mouse_xy[0],
                timeline_x_to_time(drag_x, self.view_start, self.view_end, time_lo, time_hi),
                3.0 * scale,
            )
            if preview is not None:
                self.playhead = preview
        released = self.pointer_chord is not None and not pointer.held(self.pointer_chord)
        if released:
            retime, resume = self.release(editable=editable)
            if retime is not None:
                self.retime_keyframe(ctx, *retime)
            if resume:
    ...
    def keyframes(
        ...
        cache_key = (ctx.session.keyframe_revision, self.model_id)
        if cache_key != self.keyframe_cache_key:
            self.keyframe_cache = ctx.session.model_keyframes(self.model_id)
            self.marker_index = TimelineMarkerIndex(
                tuple((key.keyframe_id, key.time) for key in self.keyframe_cache)
            )
    ...
    @staticmethod
    def can_load_keyframes(ctx: TimelineCommands) -> bool:
        return bool(
            ctx.session.paused
