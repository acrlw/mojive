...
def draw_dope_sheet(
    ...
    hit_id = -1
    if hovered and abs(mouse_xy[1] - marker_y) <= marker_radius + 4.0:
        hit_time = timeline_x_to_time(
            mouse_xy[0], editor.view_start, editor.view_end, time_lo, time_hi
        )
        hit_id = hit_keyframe(
            keyframes,
            hit_time,
            (marker_radius + 4.0)
            * max(editor.view_end - editor.view_start, _MIN_TIMELINE_SPAN)
            / time_width,
        )
    priority = []
    for key_id in (editor.selected_id, ctx.session.active_keyframe, hit_id, editor.drag_id):
        key = keyframe_by_id.get(key_id)
        if key is not None:
            priority.append(
                replace(key, time=editor.drag_preview_time)
                if key_id == editor.drag_id and editor.drag_moved
                else key
            )
    marker_positions = visible_keyframe_positions(
        keyframes,
        editor.view_start,
        editor.view_end,
        time_lo,
    ...
    snapshot_positions = {
        item.snapshot_id: timeline_time_to_x(
            item.time, editor.view_start, editor.view_end, time_lo, time_hi
        )
        for item in ctx.session.scene_snapshots
    }
    snapshot_hit = (
        hit_marker(snapshot_positions, snapshot_y, marker_radius, mouse_xy) if hovered else -1
    )
    hit = TimelineHit(
        mouse_xy,
        (time_lo, time_hi),
...
def paint_dope_sheet(
    ...
    for keyframe_id in marker_positions:
