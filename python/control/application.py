...
class ControlApplication:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed = False
        self.rpc_stats: Callable[[], dict[str, Any]] | None = None
    ...
    def _get_rpc_stats(self, _params):
        if self.rpc_stats is None:
            raise ControlError("unavailable", "This application has no RPC diagnostics provider")
    ...
    def _command(self, command) -> dict[str, Any]:
        ...
        return self._command_payload(command, result)

    def _command_payload(self, command, result) -> dict[str, Any]:
    ...
    def _edit_scene(self, params):
        expected = document_state(self.session)
        operations = []
        ...
        _check_document_precondition(self.session, expected)
        _check_pending_edits(self.session, OPERATIONS["edit_scene"])
        batch = self.session.apply_edits(
            (command for _, command in operations), label=params.get("label", "Edit scene")
        )
        if not batch.result.ok:
            details = {}
            if batch.failed_index is not None:
                details.update(
                    index=batch.failed_index, method=operations[batch.failed_index][0].name
                )
            if batch.rollback_error:
                details["rollback_error"] = batch.rollback_error
            raise ControlError(
                "rollback_failed" if batch.rollback_error else "command_failed",
                batch.result.message,
                details=details or None,
            )
        result = {**asdict(batch.result), "document": document_state(self.session)}
        result["results"] = [
            self._command_payload(command, outcome)
            for (_, command), outcome in zip(operations, batch.results, strict=True)
        ]
        return result
    ...
    def _inspect_object(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        payload.update(
            ...
            scale=self.session.scale_factors(node.node_id),
            document=document_state(self.session),
            structure_generation=self.session.structure_generation,
