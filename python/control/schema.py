...
from mojive.render.backend import DebugView, ShadowQuality
from mojive.types import CameraView, Light, LightType, Material, MeshShape
...
INSPECTED_NODE = record(
    {
        **NODE["properties"],
        ...
        "scale": {
            **array(POSITIVE, 3),
            "description": (
                "Visible local XYZ scale, including an active editor preview. "
                "Committed scale is baked into geometry and returns identity."
            ),
        },
        "document": DOCUMENT,
        "structure_generation": ID,
...
RPC_STATS_RESULT = record(
    {
        "mode": {"enum": ["headless", "viewer"]},
        "sample_limit": COUNT,
        "limits": record(
            {
                item.name: POSITIVE if item.name == "pump_budget_ms" else COUNT
                for item in fields(RpcLimits)
            }
        ),
        "counters": record(dict.fromkeys(RPC_COUNTERS, ID)),
        "timings": record(
            {
                name: record(
                    {
