...
_PENDING_EDIT_MESSAGE = "Finish the active edit or apply/discard pending model edits before changing the document remotely"
...
class Operation:
    ...
    def unavailable_reason(self, session, *, viewer_attached: bool = False) -> str | None:
        ...
        if self.writes_document and session.has_pending_model_edits:
            return _PENDING_EDIT_MESSAGE
        if (
...
_HISTORY = {"capabilities": ("edit_history",), "writes_document": True}
...
def _check_pending_edits(session, operation) -> None:
    if operation.writes_document and session.has_pending_model_edits:
        raise ControlError(
            "pending_edits",
            _PENDING_EDIT_MESSAGE,
            details={"method": operation.name},
        )
