...
class ModelEditDraft:
    ...
    def stage(self, command):
        ...
        if isinstance(command, cmd.SetGeometrySize):
            ...
            if self.pending_scale(command.node_id) is not None:
                return self.stage_scaled_size(command)
    ...
    def resolve_commands(self, session):
        ...
        if self.active and not self.compatible():
            raise ValueError("The scene changed; discard the pending model edits")
        version = EditVersion.capture(session)
        if not self._creations:
            return ModelEditPlan(tuple(self.commands), version=version)
