...
from mojive.app.composition import build, build_editor, build_scene
...
def _capture_gesture_ownership(output: Path) -> None:
    ...
    with build(
        ...
        control_modifier = imgui.Key.mod_ctrl
...
def _capture_keyframes(output: Path) -> None:
    ...
    try:
        ...
        if keyframes is not None and model_ids:
            ...
            if result.ok:
                keyframes.editor.selected_id = result.entity_id
                keyframes.editor.selection_generation = -1
        _settle(viewer, 3)
        _save_window_crop(
            viewer,
            "Keyframes",
