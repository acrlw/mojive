...
def arc_ribbon_mesh(
    ...
    *,
    ...
    """Return the arc boundary and strip/cap indices, with no concave triangulation.

    The boundary is also the mesh vertex array. Apply AA only along that boundary.
    Cap fitting uses the available arc length, independently of sampling density.
    """
...
def _arc_ribbon_shape(
    ...
    *,
    ...
    result = np.vstack((side_a, end_cap, side_b[::-1], start_cap))
    result.setflags(write=False)
    return result, _arc_ribbon_indices(len(side_a), len(end_cap), len(start_cap))
