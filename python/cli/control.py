...
def _read_rpc_limits(path):
    from mojive.control.rpc import RpcError, RpcLimits
    ...
    try:
        ...
        return RpcLimits(**values)
    except (OSError, ValueError, TypeError) as error:
        raise RpcError("invalid_params", f"Invalid RPC limits: {error}") from error


def _viewer_rpc_options(args):
    from mojive.control.rpc import RpcError

    path = getattr(args, "rpc_socket", None)
    limits_path = getattr(args, "rpc_limits", None)
    if path is None:
        if limits_path is not None:
            raise RpcError("invalid_params", "--rpc-limits requires --rpc-socket")
        return None
    return {"socket_path": Path(path), "limits": _read_rpc_limits(limits_path)}


def cmd_rpc_serve(args: argparse.Namespace) -> int:
    from mojive.app.backends import make_adapter
    from mojive.control.rpc import ControlServer, ControlService

    limits = _read_rpc_limits(args.limits_file)
    path = _resolve(args.asset)
    service = ControlService(make_adapter(args.backend, path), path, limits=limits)
    try:
...
def cmd_control(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with RpcClient(
            Path(args.socket), args.timeout, limits=_read_rpc_limits(args.limits_file)
        ) as client:
