"""Shared font discovery, verified downloads and font-source metadata."""
...
def fetch(
    remote: Remote, *, timeout: float = DOWNLOAD_TIMEOUT_S, allow_download: bool = True
) -> tuple[Path | None, str]:
    ...
    if not allow_download:
        return None, f"{remote.label} is unavailable and downloads are disabled"
    try:
...
def _resolve(
    ...
    *,
    ...
    def remote_first() -> tuple[tuple[str, int] | None, str]:
        path, why = fetch(remote, timeout=timeout, allow_download=allow_download)
        notes.append(why)
...
class FontSource:
    """Resolved font location; the font consumer owns loading and lifetime."""
...
def resolve_font_sources(
    *, allow_download: bool = False, timeout: float = DOWNLOAD_TIMEOUT_S
) -> tuple[FontSource, ...]:
    """Use the UI font policy without importing ImGui or implicitly fetching files.

    Verified cache entries remain usable when downloads are disabled. No available
    primary font is an explicit error; an unavailable CJK fallback is omitted and
    unsupported text is subsequently reported by the shaping provider.
    """
    notes = []
    mono, label = _resolve(
        _MONO,
    ...
    if mono is None:
        raise FileNotFoundError("No independent primary font is available: " + "; ".join(notes))
    sources = [FontSource(*mono, label)]
    ...
    if cjk is not None:
        sources.append(FontSource(*cjk, label))
