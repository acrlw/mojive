...
def _viewer_layout_path(config: ViewerConfig | None, *, vsync: bool) -> str:
    ...
    from mojive.ui.window import layout_settings_path
...
class Viewer:
    ...
    def configure_geometry_style(self, value: GeometryStyle, *, persist: bool = False) -> None:
        ...
        if not self.app.set_geometry_style(value, persist=persist):
            raise RuntimeError(f"The {self.backend.caps.name} backend rejected geometry style")
    ...
    def start_rpc(
        ...
        if self._rpc_server is not None:
            if limits is not None and limits != self._rpc_server.service.limits:
                raise ValueError("RPC is already running with different limits")
            return self._rpc_server
        from mojive.control.rpc import DEFAULT_SOCKET, RpcLimits, ViewerRpcServer

        self._rpc_server = ViewerRpcServer(
            self,
            DEFAULT_SOCKET if socket_path is None else socket_path,
            limits=RpcLimits() if limits is None else limits,
        )
        return self._rpc_server
...
def _compose(
    ...
    *,
    ...
    session = None
    try:
        window = create_window(window_config, renderer)
