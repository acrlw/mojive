...
def test_production_severity_stays_centered_inside_icon_library_boundary(name):
    clearance, center_x, center_y, _ink_x, _ink_y = probe._icon_review_metrics(name)
...
def test_production_information_and_warning_ink_are_mirrored():
    ...
    assert info[3] == pytest.approx(0.0, abs=1e-6)
    assert warning[3] == pytest.approx(0.0, abs=1e-6)
    assert info[4] == pytest.approx(-warning[4], abs=1e-6)
