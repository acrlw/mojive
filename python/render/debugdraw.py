...
from mojive.geometry2d.curves import CORNER_SMOOTHING, arrow_triangles
...
class Layer:
    ...
    def triangles(self, ident: str, vertices, color, duration: float = NEVER) -> None:
        ...
        Vertices have shape [N, 3, 3]; colors are RGB(A) or [N, 4].
        Input validation precedes allocation so invalid updates preserve geometry.
        """
        points = np.asarray(vertices, np.float32)
        if points.ndim != 3 or points.shape[1:] != (3, 3) or not np.isfinite(points).all():
            raise ValueError("Triangle vertices must be finite [N, 3, 3]")
        colors = np.asarray(color, np.float32)
        if colors.shape == (3,):
            colors = np.append(colors, np.float32(1))
        if colors.shape not in ((4,), (len(points), 4)) or not np.isfinite(colors).all():
            raise ValueError("Triangle colors must be finite RGBA or [N, 4]")
        if not math.isfinite(duration):
            raise ValueError("Triangle duration must be finite")
        if not len(points):
            self.erase(ident)
            return
        index = self._alloc(PrimitiveType.TRIANGLE, ident, len(points), duration)
        if index < 0:
            return
        store = self._stores[PrimitiveType.TRIANGLE]
        store.positions[index : index + len(points)] = points
        store.colors[index : index + len(points)] = colors
...
class DebugDraw:
    ...
    @staticmethod
    def _pack_triangle(dst: np.ndarray, at: int, st: _Store) -> int:
        n = st.count
        dst[at : at + n] = st.triangle_records[:n]
        return at + n
