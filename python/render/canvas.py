...
from mojive.geometry2d import Affine2D, Path2D, PathBuilder2D, StrokeStyle, flatten_path
from mojive.render.canvas_geometry import (
    PathCache,
    ellipse_points,
    polygon_path,
...
class CanvasLayer2D:
    ...
    def __init__(self, canvas: Canvas2D, layer: Layer, depth: float) -> None:
        ...
        if not math.isfinite(self.depth):
            raise ValueError("Canvas depth must be finite")
    ...
    def world(self, point) -> np.ndarray:
        ...
        return self._canvas.world(self._transform.apply(point), depth=self.depth)

    def _points(self, points) -> np.ndarray:
        value = self._transform.apply(np.asarray(points, np.float32).reshape(-1, 2))
        return (
            self._canvas.origin
    ...
    def polygon(
        ...
        *,
        ...
        """Create or update a polygon, including concave filled polygons."""
        if filled:
            self.fill_path(ident, polygon_path(points), color, duration=duration)
            return
    ...
    def transformed(self, transform: Affine2D):
        """Apply a nested local transform to subsequently authored geometry.

        Existing primitives are unchanged. Screen-pixel widths and text sizes
        retain their screen size; stroke_path widths transform with the shape.
        """
        previous = self._transform
        self._transform = previous @ transform
        try:
            yield self
        finally:
            self._transform = previous

    def triangles(self, ident, vertices, color, duration=NEVER):
        """Batch filled triangles [N, 3, 2], with one RGBA or per-triangle colors."""
        points = np.asarray(vertices, np.float32)
        if points.ndim != 3 or points.shape[1:] != (3, 2):
            raise ValueError("Canvas triangles require [N, 3, 2] vertices")
        self._layer.triangles(ident, self._points(points).reshape(-1, 3, 3), color, duration)

    def fill_path(self, ident, path: Path2D, color, *, tolerance=0.01, duration=NEVER):
        """Fill a path, including holes (evenodd/nonzero), with cached triangulation.

        Tolerance is in local canvas units. Author once and keep the retained ID;
        do not resubmit unchanged geometry each frame.
        """
        mesh = self._canvas._paths.prepare(path, tolerance)
        self.triangles(ident, mesh, color, duration)
    ...
    def stroke_path(
        ...
        *,
        ...
        """Stroke a path in canvas units, with joined, non-overlapping geometry.

        Unlike line/polyline width_px, this width scales with the world plane.
        Caps: butt/round/square. Joins: miter/round/bevel.
        """
        style = StrokeStyle(width=width, cap=cap, join=join, miter_limit=miter_limit)
        mesh = self._canvas._paths.prepare(path, tolerance, style)
        self.triangles(ident, mesh, color, duration)
    ...
    def arc(
        ...
        *,
        ...
        """Open elliptical arc; start, signed sweep, and rotation are radians."""
        points = ellipse_points(center, radii, start, sweep, segments, rotation)
        self.polyline(ident, points, color, width_px, duration=duration)

    def bezier(self, ident, points, color, width_px=1.5, *, tolerance=0.01, duration=NEVER):
        """Quadratic (3 points) or cubic (4 points), sampled in canvas units."""
        points = np.asarray(points, np.float64)
        if points.shape not in ((3, 2), (4, 2)):
            raise ValueError("Bezier requires three or four 2D control points")
        builder = PathBuilder2D().move_to(*points[0])
        if len(points) == 3:
            builder.quadratic_to(*points[1:].ravel())
        else:
            builder.cubic_to(*points[1:].ravel())
        contour = flatten_path(builder.finish(), tolerance=tolerance, max_points=65536)[0]
        self.polyline(ident, contour.points, color, width_px, duration=duration)
    ...
    def rounded_rectangle(
        ...
        *,
        ...
        path = rounded_rectangle_path(lo, hi, radius)
        if filled:
            self.fill_path(ident, path, color, tolerance=tolerance, duration=duration)
        else:
            contour = flatten_path(path, tolerance=tolerance, max_points=65536)[0]
            self.polyline(ident, contour.points, color, width_px, closed=True, duration=duration)
    ...
    def __init__(
        ...
        *,
        ...
        if not np.isfinite(self.origin).all():
            raise ValueError("Canvas origin must be finite")
