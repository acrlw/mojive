...
static FrameToken present(Renderer &r, Target target, Extent size, Texture tex) {
    ...
    return r.render_ui({size, vertices, indices, commands}, target);
}
int main(int argc, char **argv) {
    try {
        ...
        auto renderer = probe::make_renderer({probe::native_window(window), argv[1]}, argv[3]);
        std::array<std::byte, 16> pixels{
            std::byte(255), std::byte(0),   std::byte(0),   std::byte(255),
            std::byte(0),   std::byte(255), std::byte(0),   std::byte(255),
            std::byte(0),   std::byte(0),   std::byte(255), std::byte(255),
            std::byte(255), std::byte(255), std::byte(255), std::byte(255)};
        auto texture = renderer->upload_texture({2, 2}, pixels);
        auto capture = renderer->create_target({320, 240}, 1);
        ...
        for (int cycle = 0; cycle < cycles; ++cycle) {
            ...
            auto surface = renderer->create_surface(probe::native_window(peer));
            for (int frame = 0; frame < 18; ++frame) {
                if (frame == 5) {
                    glfwSetWindowSize(peer, 360 + cycle % 4 * 37, 230 + cycle % 3 * 29);
                    glfwPollEvents();
                    renderer->resize(surface, probe::native_window(peer).size);
                    ++resizes;
                }
                if (frame == 9) {
                ...
                glfwPollEvents();
                present(*renderer, {}, probe::native_window(window).size, texture);
                // A minimized or hidden platform window has no presentation demand.
                if (!glfwGetWindowAttrib(peer, GLFW_ICONIFIED))
                    present(*renderer, surface, probe::native_window(peer).size, texture);
                renderer->advance();
            ...
            auto image = wait_for_readback(*renderer, renderer->readback(token, Product::Color));
