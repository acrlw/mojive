...
int main(int argc, char **argv) {
    try {
        ...
        auto renderer = probe::make_renderer({{}, argv[1]}, argv[3]);
        ...
        auto texture = renderer->upload_texture({8, 8}, pixels);
        ...
        auto destination = renderer->create_target({128, 128}, 1);
        auto source = renderer->create_target({128, 128}, 1);
        auto mask = renderer->create_target({128, 128}, 1);
        Quad original(texture, {128, 128});
        renderer->render_ui(original.frame(), source);
        Quad copy(renderer->target_texture(source), {128, 128});
        auto copied = renderer->render_ui(copy.frame(), destination);
        auto result = wait_for_readback(*renderer, renderer->readback(copied, Product::Color));
        ...
        auto blended = renderer->render_ui(blend.frame(), mask);
        result = wait_for_readback(*renderer, renderer->readback(blended, Product::Color));
        ...
        for (int n = 0; n < 120; ++n) {
            ...
            auto tex = renderer->upload_texture({8, 8}, pixels);
            auto b = renderer->create_target(size, 1), a = renderer->create_target(size, 1);
            Quad q(tex, size);
            renderer->render_ui(q.frame(), a);
            Quad dependent(renderer->target_texture(a), size);
            auto frame = renderer->render_ui(dependent.frame(), b);
            auto ticket = renderer->readback(frame, Product::Color, {12, 12, 1, 1});
            if (n % 3 == 0) {
                renderer->resize(b, {64, 64});
                if (wait_for_readback(*renderer, ticket).state != ReadbackState::Canceled)
                    throw std::runtime_error("Resize did not cancel composition readback");
            } else
                expect(wait_for_readback(*renderer, ticket).image, 0, 0, {255, 0, 0});
            renderer->destroy(a);
            renderer->destroy(b);
