...
class SdlRenderer final : public Renderer {
  ...
  public:
    ...
    void setVsync(Target id, bool enabled) override {
        ...
        auto mode = enabled ? SDL_GPU_PRESENTMODE_VSYNC : SDL_GPU_PRESENTMODE_IMMEDIATE;
        if (!SDL_WindowSupportsGPUPresentMode(mDevice, t.window, mode))
            mode = SDL_GPU_PRESENTMODE_VSYNC;
        submit();
        checked(SDL_SetGPUSwapchainParameters(mDevice, t.window, SDL_GPU_SWAPCHAINCOMPOSITION_SDR,
                                              mode));
    }
