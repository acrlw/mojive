...
void RenderRuntime::setOverlays(Scene scene, OverlayFrame frame) {
    return mImpl->invoke([&](Renderer &r) { return r.setOverlays(scene, std::move(frame)); });
}
void RenderRuntime::setLighting(Scene scene, const Lighting &lighting) {
    return mImpl->invoke([&](Renderer &r) { return r.setLighting(scene, lighting); });
}
...
void RenderRuntime::setVsync(Target target, bool enabled) {
    return mImpl->invoke([&](Renderer &r) { return r.setVsync(target, enabled); });
}
...
FrameToken RenderRuntime::renderRequested(Target target, const CameraView &camera,
                                          RenderRequest request) {
    validateCamera(camera);
    if (!request.color && !request.sceneData)
        throw std::invalid_argument("Empty render request");
    return mImpl->invoke([&](Renderer &r) { return r.renderRequested(target, camera, request); });
}
