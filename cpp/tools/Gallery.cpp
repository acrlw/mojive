    ...
    void update(Renderer &renderer) {
        ...
        for (auto *texture : ImGui::GetPlatformIO().Textures) {
            if (texture->Status == ImTextureStatus_WantCreate ||
                ...
                auto id = renderer.uploadTexture(
...
int main(int argc, char **argv) {
    try {
        ...
        options.shaderDirectory = argv[1];
        options.window = probe::nativeWindow(window);
        auto renderer = probe::makeRenderer(options, backend);
        auto scene = probe::load(argv[2]);
        renderer->setScene(scene.source);
        auto viewport = renderer->createTarget({1280, 720}, 4);
        auto capture = renderer->createTarget(options.window.size);
        auto secondarySize = probe::nativeWindow(secondary).size;
        auto surface = renderer->createSurface(probe::nativeWindow(secondary));
        ImGui::CreateContext();
        auto &io = ImGui::GetIO();
        ...
        for (uint64_t n = 0; n < 110; ++n) {
            ...
            renderer->resize(viewport, viewportSize);
            auto camera = scene.camera;
            camera.projection[0] *= (16.0f / 9) / (float(viewportSize.width) / viewportSize.height);
            renderer->update({1, n, scene.frames[n % scene.frames.size()]});
            renderer->render(viewport, camera);
            ImGui::Image(ImTextureRef(renderer->targetTexture(viewport).id), available);
            ImGui::End();
            ImGui::PopStyleVar();
            ImGui::Render();
            packets.update(*renderer);
            renderer->renderUi(packets.frame);
            renderer->resize(capture, packets.frame.size);
            captureFrame = renderer->renderUi(packets.frame, capture);
            secondarySize = probe::nativeWindow(secondary).size;
            renderer->resize(surface, secondarySize);
            std::array<UiVertex, 4> quad = {
                {{0, 0, 0, 0, 0xffffffff},
                 {float(secondarySize.width), 0, 1, 0, 0xffffffff},
                 {float(secondarySize.width), float(secondarySize.height), 1, 1, 0xffffffff},
                 {0, float(secondarySize.height), 0, 1, 0xffffffff}}};
            std::array<uint32_t, 6> indices = {0, 1, 2, 0, 2, 3};
            std::array<UiCommand, 1> command = {
                {{0,
                  6,
                  0,
                  {0, 0, float(secondarySize.width), float(secondarySize.height)},
                  renderer->targetTexture(viewport)}}};
            renderer->renderUi({secondarySize, quad, indices, command}, surface);
            renderer->advance();
            std::this_thread::sleep_for(std::chrono::milliseconds(8));
        }
        auto readback = renderer->readback(captureFrame, Product::Color);
        ReadbackResult result;
        result = waitForReadback(*renderer, readback);
        ...
        readback = renderer->readback(captureFrame, Product::Color);
        result = waitForReadback(*renderer, readback);
