...
static void conformance(Renderer &renderer, const std::filesystem::path &output) {
    ...
    CameraView camera;
    camera.view = lookAt({0, 0, 5}, {0, 0, 0}, {0, 1, 0});
    camera.projection = orthographic(4, 4, .1f, 20);
    camera.farPlane = 20;
    camera.revision = 9;
    auto target = renderer.createTarget({256, 256}, 4);
    ...
    for (size_t n = 0; n < 3; ++n) {
        ...
        require(pixel<uint32_t>(i.image, x, y) == source.instances[n].objectId,
    ...
    for (uint32_t y = 50; y < 110; ++y)
        for (uint32_t x = 35; x < 92; ++x)
            require(pixel<uint32_t>(i.image, x, y) == source.instances[0].objectId,
    ...
    auto newerFrame = renderer.render(target, camera);
    auto newer = renderer.readback(newerFrame, Product::ObjectId, {192, 83, 1, 1});
    ...
    auto peer = renderer.createTarget({128, 96});
    auto peerFrame = renderer.render(peer, camera);
    auto pending = renderer.readback(peerFrame, Product::ObjectId);
    ...
    other.join();
    require(wrongThread, "Owner-thread rule not enforced");
    ...
    report << "{\n  \"passed\": true,\n  \"backend\": \"" << caps.backend
           << "\",\n  \"integer_target_capability\": " << caps.integerTarget
           << ",\n  \"signed_pair_target_capability\": " << caps.signedPairTarget
           << ",\n  \"id_encoding\": \"lossless_rgba8\",\n  \"color_msaa\": 4,\n  "
              "\"multiple_windows_capability\": "
           << caps.multipleWindows << "\n}\n";
}
