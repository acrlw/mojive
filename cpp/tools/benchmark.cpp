...
int main(int argc, char **argv) {
    try {
        ...
        auto target = renderer->createTarget(size, 4);
            ...
            if (fpsLimit > 0)
                std::this_thread::sleep_until(
                    start + std::chrono::duration_cast<Clock::duration>(
                                std::chrono::duration<double>(frames / fpsLimit)));
        }
        while (!pending.empty()) {
            renderer->advance();
            consume(true);
        ...
        snapshot = waitForReadback(*renderer, ticket);
